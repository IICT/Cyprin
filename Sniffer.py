...
def MakeFilter(ipAddress=None, transportProtocol=None, port=None):
    ...
    if transportProtocol and not CheckTransportProtocol(transportProtocol):
        raise ValueError("Le protocole de transport doit �tre 'TCP' ou 'UDP'. Non valable: %s" % transportProtocol)
    if port and not CheckPort(port):
        raise ValueError("Le port doit �tre entre 1 et 65535. Non valable: %s" % port)
